...
function App() {
  ...
  const saveCards = useCallback((cardsData) => {
    try {
      localStorage.setItem('memory-cards', JSON.stringify(cardsData));
    } catch (error) {
  ...
  const saveCustomAttributes = useCallback((attributes) => {
    try {
      localStorage.setItem('custom-attributes', JSON.stringify(attributes));
  ...
  const loadCustomAttributes = useCallback(() => {
    try {
      const saved = localStorage.getItem('custom-attributes');
      if (saved) {
        const parsedAttributes = JSON.parse(saved);
  ...
  const loadCards = useCallback(() => {
    try {
      const saved = localStorage.getItem('memory-cards');
        if (saved) {
          ...
          console.log('从 localStorage 加载了', cardsWithDates.length, '张卡片');
          return; // 重要：如果有保存的数据就直接返回，不要创建示例卡片
        }
      } catch (error) {
